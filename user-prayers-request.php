...
Text Domain: go-prayers-request
...
{	
	...
	function upr_custom_prayers_column( $column, $post_id ) {
			switch ( $column ) {		
				...
				if($author_display_name!='' || !empty($author_display_name)){
					...
					echo '<a href="'.esc_url($authorLink).'" target="_blank">'.esc_html($author_display_name).'</a>';
				} else {
	...
	function upr_ajax_do_pray($pera){
		...
		if( isset( $_REQUEST['_do_prayer_nonce'] ) && wp_verify_nonce( $_REQUEST['_do_prayer_nonce'], 'do-prayer-nonce' ) ){
			...
			if($post_id){
				...
				esc_html_e('Prayed','wp-prayers-request'); exit;
			}
		}
	...
	function upr_ajax_pray_response(){		
		if( isset( $_REQUEST['_pray_response_nonce'] ) && wp_verify_nonce( $_REQUEST['_pray_response_nonce'], 'pray-response-nonce' ) ){
			...
			if($comment_id){
				...
				$blacklisted = wp_check_comment_disallowed_list($comment->comment_author, $comment->comment_author_email, $comment->comment_author_url, $comment->comment_content, $comment->comment_author_IP, $comment->comment_agent);
				if($blacklisted){
	...
	public function upr_prayers_management_init() {
		...
		$labels = array(
			...
			'menu_name'          => __('GoPrayer','wp-prayers-request'),
	...
	public function upr_enqueue_public_scripts_and_styles(){
		...
		if( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'upr_list_prayers') || has_shortcode( $post->post_content, 'upr_form') ) {
			wp_enqueue_script('upr_scripts', plugin_dir_url(__FILE__). '/assets/upr_prayers.js', array(), '1.0.0', true);
			wp_enqueue_style('upr_frontend', plugin_dir_url(__FILE__). '/assets/css/frontend.css', array(), '1.0' );
		}}	
// 		wp_enqueue_style('upr_frontend', plugin_dir_url(__FILE__). '/assets/css/frontend.css', array(), '1.0' );
	}
