...
//         if ((isset($_REQUEST['g-recaptcha-response']) && ! empty($_REQUEST['g-recaptcha-response'])) || $upr_prayer_hide_captcha !=1) {
		if ( ((isset($_REQUEST['g-recaptcha-response']) && ! empty($_REQUEST['g-recaptcha-response'])) || $upr_prayer_hide_captcha !=1) && ( isset( $_REQUEST['_prayer_request_nonce'] ) && wp_verify_nonce( $_REQUEST['_prayer_request_nonce'], 'prayer-request-nonce' ) ) ) {
            ...
            if($upr_prayer_hide_captcha==1) {
              ...
              $curl = wp_remote_get();
              wp_remote_get($curl, CURLOPT_URL, $url);
              wp_remote_get($curl, CURLOPT_RETURNTRANSFER, true);
              wp_remote_get($curl, CURLOPT_TIMEOUT, 15);
              wp_remote_get($curl, CURLOPT_SSL_VERIFYPEER, true);
              wp_remote_get($curl, CURLOPT_SSL_VERIFYHOST, true);
              $curlData = wp_remote_get($curl);
...
//							$to = $prayer_author_info->user_email;
                            ...
                            $message = $upr_prayer_thankyou;} else {
                            $message = __('Thank you. Your form has been received','wp-prayers-request');}
        ...
        <form id="form_gc" method="post" action="">
            ...
            <div class="wpgmp-frontend lxt-kewal">
                ...
                if (isset($message)) {
                    echo esc_html($message);echo '<script>window.history.replaceState( null, null, window.location.href );</script>';
                }
                ...
                if ($showform) { 
					wp_nonce_field( 'prayer-request-nonce', '_prayer_request_nonce' );
				?>
                    <div class="form-group ">
                        <div class="col-md-3"><label for="prayer_author_name"><?php esc_html_e('Name',
                    ...
                    class="form-control" type="text" value="<?php if (isset($prayername)) {echo  esc_html($prayername);}?>"
                            <p class="help-block" id="prayer_author_name_error" maxlength="20"></p></div>
                    </div>
				<?php if ($upr_prayer_req_email == 0): ?>	
                    <div class="form-group ">
                        <div class="col-md-3"><label for="prayer_author_email"><?php esc_html_e('Email',
                                    'wp-prayers-request'); ?></label><span class="inline-star">*</span></div>
                        <div class="col-md-8"><input name="prayer_author_email" id="prayer_author_email" maxlength="30"
                        class="form-control" type="email" 
                            <p class="help-block" id="prayer_author_email_error"><?php esc_html_e('This field is required',
                ...
                <?php if ($upr_show_prayer_category == 1): ?>
                    <div class="form-group ">
                        ...
                        <div class="col-md-3"><label for="prayer_author_category"><?php esc_html_e('Category',
                ...
                ?>
                    <div class="form-group ">
                        <div class="col-md-3"><label for="prayer_author_country"><?php esc_html_e('Country',
                                    'wp-prayers-request') ?></label></div>
                        <div class="col-md-8">
                            <select name="prayer_author_country" class="form-control">
                                <?php
                                foreach ($select_country as $key => $val) {
                                    echo '<option value="'.esc_html($val).'">'.esc_html($val).'</option>';
                                }
                                ?>
                            </select>
                        </div>
                ...
                <?php endif; ?>
                    <div class="form-group ">
                        <div class="col-md-3"><label for="prayer_messages"><?php esc_html_e('Prayer Request',
                                    'wp-prayers-request') ?></label><span class="inline-star">*</span></div>
                        <div class="col-md-8"><textarea rows="5" name="prayer_messages" id="prayer_messages"
                        class="form-control" maxlength="500"></textarea>
                            <p class="help-block" id="prayer_messages_error"><?php esc_html_e('This field is required',
                                    'wp-prayers-request') ?></p></div>
                    </div>
                    ...
                    class="form-control" type="checkbox" value="prayer_public"><?php esc_html('Do not share this request',
                                    'wp-prayers-request') ?>
                ...
                <?php if ($upr_prayer_hide_captcha == 0): ?>
                    <!--<div class="form-group ">-->
                    <!--   	<div class="col-md-3"><label for="prayer_captcha"><?php esc_html_e('Captcha',
                        'wp-prayers-request') ?></label><span class="inline-star">*</span></div>-->
                    <!--       <div class="col-md-8">-->
                    <!--           <div class="col-md-3">-->
                    ...
                    <!--    <div class="col-md-8">-->
                    <!--    	<p class="help-block"><?php esc_html_e("Enter here captcha. Can't read the image?",
                        'wp-prayers-request') ?> <?php esc_html_e("Click",
                        'wp-prayers-request') ?> <a href="javascript: refreshCaptcha();"><?php esc_html_e("HERE",
                        'wp-prayers-request') ?></a> <?php esc_html_e("to refresh", 'wp-prayers-request') ?>.</p>            	</div>-->
                    <!--</div> -->
                ...
                <?php endif; ?>
                    ...
                    <div class="form-group ">
                        ...
                        <div class="col-md-12"><input name="save_prayer_data" id="save_prayer_data"
                                                      class="btn btn-primary" value="<?php esc_html_e("Submit", 'wp-prayers-request') ?>"
                                                      type="submit"></div>
                    </div>
                <?php } else { ?>
                    <p><?php esc_html_e('Sorry, you have to login first to request a prayer.', 'wp-prayers-request'); ?>
                        . <?php esc_html_e('Click to', 'wp-prayers-request'); ?> <a
                                href="<?php echo esc_url(wp_login_url($current_url)) ?>"><?php esc_html_e('login', 'wp-prayers-request'); ?></a>.
                    </p>
        ...
        <script>
            jQuery(document).ready(function ($) {
			 ...
			 var anyname = "<?php echo esc_js($praybutton1); ?>";
        </script>
    <?php endif; ?>
        <?php
...
<?php endif; ?>
    ...
    <div class="comment-meta commentmetadata"><a
			...
			printf(esc_html(human_time_diff( get_comment_time( 'U' ), current_time('U'))));			
    ...
    if ($upr_allow_comments_prayer_request == 1) {
        if (comments_open($comment->comment_post_ID)) {
            if (is_user_logged_in()) {
                echo '<div class="reply"><a class="comment-reply-link pray-replay" id="'.esc_html($comment->comment_ID).'">'.esc_html(__('Reply'),
                        'wp-prayers-request').'</a></div>';
            } else {
                if ($registration == 1):
                    echo '<div class="reply"><a href="'.esc_url(wp_login_url($current_url)).'" id="'.esc_url($current_url).'">'.esc_html(__('Log in to Reply',
                            'wp-prayers-request')).'</a></div>';
                else:
                    echo '<div class="reply"><a class="comment-reply-link pray-replay" id="'.esc_html($comment->comment_ID).'">'.esc_html(__('Reply',
                            'wp-prayers-request')).'</a></div>';
    ...
    <div id="reply_<?php comment_ID() ?>" class="comment-respond" style="display:none">
        <form action="/action_page.php" method="post" class="comment-form">
            <p class="comment-form-comment">
                <label for="comment"><?php esc_html_e('Comment', 'wp-prayers-request') ?> <a id="<?php comment_ID() ?>"
                                                                          class="cancelcomment"><?php esc_html_e('Reply',
            ...
            <?php if ( ! is_user_logged_in()) { ?>
                <p class="comment-form-author">
                    <label for="author"><?php esc_html_e('Name', 'wp-prayers-request') ?></label>
                    <input id="author_<?php comment_ID() ?>" name="author" type="text" value="" size="30"></p>
                <p><label for="author"><?php esc_html_e('Email', 'wp-prayers-request') ?> </label>
                    <input id="email_<?php comment_ID() ?>" name="email" type="email" value="" size="30"></p>
                <p><label for="author"><?php esc_html_e('Website', 'wp-prayers-request') ?></label>
            ...
            <p class="form-submit">
                <input name="submit" id="<?php comment_ID() ?>" class="submit prayresponsreply"
                       value="<?php esc_html_e('Submit', 'wp-prayers-request') ?>" type="button">
...
function upr_get_prayers_output($arguments = '')
{
    ...
    if ($upr_prayer_fetch_req_from != 'all') {
        $prayers_args = array(
            ...
            'order' => 'DESC',
            'date_query' => array(
                'after' => gmdate('Y-m-d', strtotime('-'.$upr_prayer_fetch_req_from.' days')),
            ),
        );
    } else {
        $prayers_args = array(
    ...
    if ($prayers) {
		wp_nonce_field( 'do-prayer-nonce', '_do_prayer_nonce' );
		wp_nonce_field( 'pray-response-nonce', '_pray_response_nonce' );
        $current_url = get_permalink(get_the_ID());
        ...
        foreach ($prayers as $prayer) {
            ...
            $prayer_date=human_time_diff( get_the_time('U', $wp_prayer_id), current_time('U') );
...
//             );
            ...
            if ($upr_display_username == 1) {
                ...
                $html .= '<div class="postmeta">'.$submittedby.($prayer_date).' | ';
            } else {
                $html .= '<div class="postmeta">'.($prayer_date).' | ';
            }
