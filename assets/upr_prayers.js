...
jQuery(document).ready(function ($) {
    ...
    $(document).on("click", '.prayresponse', function () {
		...
		var pray_response_nonce = jQuery("#_pray_response_nonce").val();
        var ajax_url = $("#admin-ajax").val();
        $.ajax({
            url: ajax_url,
            type: 'post',
            data: {
                ...
                comment_author_email: comment_author_email,
                comment_author_url: comment_author_url,
				_pray_response_nonce: pray_response_nonce
            },
            success: function (response) {
                $('#commentlist_' + prayer_id).slideToggle('slow');
                window.location.href = url + '/#commentlist_' + prayer_id;
...
function do_pray(pray_id, time_interval, user_id) {
	...
	var do_pray_nonce = jQuery("#_do_prayer_nonce").val();
    jQuery.ajax({
        url: ajax_url,
        type: 'post',
        data: {
            action: 'ajax_do_pray',
            prayer_id: pray_id,
			_do_prayer_nonce: do_pray_nonce
        },
        beforeSend: function () {
